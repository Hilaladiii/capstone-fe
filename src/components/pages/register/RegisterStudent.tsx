...
const signUpSchema = z.object({
  ...
  nim: z.string().min(1, "NIM wajib diisi").length(15, "NIM harus terdiri dari 15 karakter"), 
  username: z.string().min(1, "Username wajib diisi").min(4, "Username harus lebih dari 3 karakter"),
...
const RegisterStudent = () => {
  ...
  return (
    <div className="max-h-screen flex overflow-hidden">
        ...
        <div className="w-240 h-240 bottom-52 right-48 rounded-full bg-gradient-to-b from-[#131E30] to-[#17294C] border-6 border-secondary text-white flex flex-col justify-center items-center p-4 relative">
          <div className="z-10 absolute top-2/4 left-4/12">
            ...
            <p className="text-sm leading-relaxed max-w-md">
              PKLapps adalah aplikasi yang memfasilitasi mahasiswa dalam proses ihgouguou whfiwufwifo jfeifefhefn eifheufjwnclfwofu efihwueghw3jg weoifhwug3hfjw ihefughquwfg uahwgfiygqwf woif hrwufg efhweiufgh
            </p>
          </div>
      ...
      <section className="w-1/2 flex-col items-center justify-center mb-10 pt-8 pb-20 overflow-auto">
        <div className="w-full h-3/4 ">
          <h2 className="text-3xl font-bold text-secondary mb-2">Sign Up</h2>
          <p className="font-semibold text-base mb-6">Isi form dibawah untuk membuat akun baru</p>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-2 ml-2">
            <Input 
              register={register} 
              name="email" 
              label="Email UB" 
            ...
            <Input 
              ...
              errors={errors.nim} 
            />
            {errors.nim && <p className="text-red-500 text-sm">{errors.nim.message}</p>}
            <Input 
              register={register} 
              name="sks" 
              label="SKS"
              ...
              errors={errors.sks} 
            />
            {errors.sks && <p className="text-red-500 text-sm">{errors.sks.message}</p>}
            <Input 
              register={register} 
              name="year" 
              label="Tahun" 
          ...
          <div className="mt-4 text-center">
            <p className="text-base font-semibold">
              ...
              <Link to="/login" className="pr-40 text-secondary font-semibold hover:underline">Login sekarang</Link>
            </p>
          </div>
        </div>
      </section>
