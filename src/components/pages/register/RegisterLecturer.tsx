...
import { registerLecturer } from "../../../services/register.service";
...
const registerLecturerSchema = z.object({
  ...
  nip: z.string().min(1, "NIP wajib diisi").length(16, "NIP harus terdiri dari 16 karakter"),
});

type TSignUp = z.infer<typeof registerLecturerSchema>;
...
const RegisterLecturer = () => {
  ...
  const onSubmit = async (data: TSignUp) => {
    try {
      const result = await registerLecturer(data); 
        ...
        <div className="w-240 h-240 bottom-52 right-48 rounded-full bg-gradient-to-b from-[#131E30] to-[#17294C] border-6 border-secondary text-white flex flex-col justify-center items-center p-4 relative">
          <div className="z-10 absolute top-2/4 left-4/12">
            ...
            <p className="text-sm leading-relaxed max-w-md">
              PKLapps adalah aplikasi yang memfasilitasi dosen dalam proses ihgouguou whfiwufwifo jfeifefhefn eifheufjwnclfwofu efihwueghw3jg weoifhwug3hfjw ihefughquwfg uahwgfiygqwf woif hrwufg efhweiufgh
            </p>
          </div>
