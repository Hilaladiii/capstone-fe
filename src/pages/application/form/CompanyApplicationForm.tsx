...
import { InternshipApplication } from "../../../common/types/internship.type";
  ...
  return (
    <main className="flex flex-col w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-center items-center min-w-6xl mx-auto py-6 space-y-4">
        ...
        <Input name="recipientOfLetter" label="Penerima Surat" register={register} errors={errors.recipientOfLetter} placeholder="Masukkan penerima surat dari instansi yang anda lamar" className="min-w-6xl w-full" />

        <Input name="agencyName" label="Nama Instansi" register={register} errors={errors.agencyName} placeholder="Masukkan nama instansi yang anda lamar" className="min-w-6xl w-full" />

        <Input name="agencyAddress" label="Alamat Instansi" register={register} errors={errors.agencyAddress} placeholder="Masukkan alamat instansi anda (nama jalan, kecamatan, kabupaten/kota, provinsi)" className="min-w-6xl w-full" />

        <Input name="internshipObject" label="Objek PKL" register={register} errors={errors.internshipObject} placeholder="Masukkan objek pkl yang anda ajukan (bisa diisi dengan mata kuliah terkait)" className="min-w-6xl w-full" />

        <Input name="startDate" label="Tanggal mulai PKL" register={register} errors={errors.startDate} type="datetime-local" className="min-w-6xl w-full" />

        <Input name="finishDate" label="Tanggal selesai PKL" register={register} errors={errors.finishDate} type="datetime-local" className="min-w-6xl w-full" />

        <div className="min-w-6xl w-full font-semibold">
          <FileUpload name="studyResultCardFile" label="KHS Semester Akhir" file={watch("studyResultCardFile") ?? undefined} register={register} setValue={setValue} error={errors.studyResultCardFile} className="min-w-6xl w-full" />
        </div>
