...
const Login = () => {
  ...
  return (
    <section className="w-full flex flex-col items-center justify-center pt-8 pb-20 overflow-auto">
      <FormAuthLayout title="Sign In" description="Isi form dibawah untuk login ke akun anda">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-2 ml-2">
          ...
          <div className="flex items-center gap-2 mt-3">
            <input type="checkbox" id="rememberMe" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="w-4 h-4 text-white bg-white border-2 border-black rounded accent-black cursor-pointer" />
            <label htmlFor="rememberMe" className="text-sm font-normal text-black">
              Remember me
            </label>
          </div>
        ...
        <div className="w-full mt-4 text-center text-sm font-semibold">
          ...
          <Link to="/auth/sign-up" className="text-secondary font-semibold hover:underline">
            Daftar sekarang
          </Link>
        </div>
      </FormAuthLayout>
